...
export const GalaxyGenerator = {
  ...
  loop(diff) {
    ...
    if (this.sacrificeActive) {
      this.capRift.reducedTo = Decimal.max(Decimal.sub(this.capRift.reducedTo, diff.div(1e3).mul(0.075))).toNumber();
    ...
    if (!this.capRift) {
      PelleRifts.all.forEach(r => r.reducedTo = diff.div(1e3).mul(0.05).add(r.reducedTo).clampMax(2).toNumber());
